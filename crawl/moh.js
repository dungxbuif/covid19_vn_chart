...
const getLocalVN = async () => {
    try {
        ...
        for (let i = 0; i < script.length; i++) {
            if (
                script[i].children[0].data.includes('var data = [{"socakhoi"')
            ) {
        ...
        console.log(
            errorWaring(
                `Get MOH source fail because ${err} at ${moment(
                    new Date()
...
const getDetailVN = async () => {
    try {
        ...
        console.log(
            successAlert(
                `Get VN detail source successfully at ${moment(
                    new Date()
                ).format("DD/MM/YYYY HH:mm:SS")}`
            )
        );
...
const getData = async () => {
    return new Promise(async (resolve, reject) => {
        resolve({
            local_vn: await getLocalVN(),
            vietnam: await getDetailVN(),
        });
    });
};
